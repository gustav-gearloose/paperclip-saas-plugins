...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    if (!clientIdRef || !clientSecretRef || !refreshTokenRef || !tenantId) {
      ctx.logger.error("xero plugin: clientIdRef, clientSecretRef, refreshTokenRef, and tenantId are required");
      return;
    }
    ...
    try {
      ...
      ctx.logger.error(`xero plugin: failed to resolve secrets: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }

    const client = new XeroClient(clientId, clientSecret, refreshToken, tenantId);
